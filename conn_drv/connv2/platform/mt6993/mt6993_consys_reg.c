...
#include "osal.h"
#include "include/mt6993_consys_reg.h"
#include "include/mt6993_consys_reg_offset.h"
#include "include/mt6993_debug_gen.h"
#include "include/mt6993_pos.h"
#include "include/mt6993_pos_gen.h"

#define CONSYS_DUMP_BUF_SIZE 1000
struct consys_base_addr g_conn_reg_mt6993;
static struct conn_debug_info_mt6993 *g_debug_info_ptr;
static struct conn_debug_info_mt6993 *g_debug_top_power_info_ptr;
...
int consys_reg_init_mt6993(struct platform_device *pdev)
{
	int ret = -1;
	...
	if (node) {
		for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
			base_addr = &g_conn_reg_mt6993.reg_base_addr[i];
...
int consys_reg_deinit_mt6993(void)
{
	int i = 0;

	for (i = 0; i < CONSYS_BASE_ADDR_MAX; i++) {
		if (g_conn_reg_mt6993.reg_base_addr[i].vir_addr) {
			pr_info("[%d] Unmap %s (0x%zx)",
				i, consys_base_addr_index_to_str[i],
				g_conn_reg_mt6993.reg_base_addr[i].vir_addr);
			iounmap((void __iomem*)g_conn_reg_mt6993.reg_base_addr[i].vir_addr);
			g_conn_reg_mt6993.reg_base_addr[i].vir_addr = 0;
		}
	}
...
int consys_check_ap2conn_infra_on_mt6993(void)
{
	//don't check ap2onn slpprot since it is disabled while conninfra is power on.
	return 0;
}
...
int consys_check_ap2conn_infra_off_clock_mt6993(void)
{
...
int consys_check_ap2conn_infra_off_irq_mt6993(void)
{
...
int consys_check_reg_readable_mt6993(void)
{
#ifdef CONFIG_FPGA_EARLY_PORTING
	return 1;
#endif
	if (consys_check_ap2conn_infra_on_mt6993())
		return 0;
	if (consys_check_ap2conn_infra_off_clock_mt6993())
		return 0;
	if (consys_check_ap2conn_infra_off_irq_mt6993())
		return 0;
	return 1;
}

int consys_check_reg_readable_for_coredump_mt6993(void)
{
...
int consys_is_consys_reg_mt6993(unsigned int addr)
{
...
int consys_is_bus_hang_mt6993(void)
{
	int ret = 0;
	int is_clock_fail;
	...
	pr_notice("[CONN_BUS] version=%s\n", MT6993_CONN_INFRA_BUS_DUMP_VERSION);
	consys_print_power_debug_dbg_level_0_mt6993_debug_gen(
		CONNINFRA_BUS_LOG_LEVEL_HOST_ONLY, g_debug_info_ptr);
	consys_print_log("[CONN_POWER_A]", g_debug_info_ptr);

	ret = consys_check_ap2conn_infra_on_mt6993();
	if (ret) {
		pr_notice("[%s] ap2conn_infra_on fail", __func__);
		return ret;
	}
	consys_print_bus_slpprot_debug_dbg_level_0_mt6993_debug_gen(
		CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON, g_debug_info_ptr);
	consys_print_log("[slpprot_a]", g_debug_info_ptr);
	consys_print_power_debug_dbg_level_1_mt6993_debug_gen(
		CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON, g_debug_info_ptr);
	consys_print_log("[CONN_POWER_B]", g_debug_info_ptr);
	consys_print_bus_debug_dbg_level_1_mt6993_debug_gen(
		CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON, g_debug_info_ptr);
	consys_print_log("[CONN_BUS_B]", g_debug_info_ptr);
	consys_print_infra_top_cr_debug_dbg_level_1_mt6993_debug_gen(
		CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON, g_debug_info_ptr);
	consys_print_log("[TOP_CR_B]", g_debug_info_ptr);

	is_clock_fail = consys_check_ap2conn_infra_off_clock_mt6993();
	if (is_clock_fail)
		pr_notice("[%s] ap2conn_infra_off clock fail", __func__);

	ret = consys_check_ap2conn_infra_off_irq_mt6993();
	if (ret)
		pr_notice("[%s] ap2conn_infra_off timeout irq fail", __func__);

	if (!is_clock_fail) {
		consys_print_power_debug_dbg_level_2_mt6993_debug_gen(
			CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF, g_debug_info_ptr);
		consys_print_log("[CONN_POWER_C]", g_debug_info_ptr);

		consys_print_bus_debug_dbg_level_2_mt6993_debug_gen(
			CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF, g_debug_info_ptr);
		consys_print_log("[CONN_BUS_C]", g_debug_info_ptr);

		consys_print_bus_slpprot_debug_dbg_level_2_mt6993_debug_gen(
			CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF, g_debug_info_ptr);
		...
		consys_print_infra_top_cr_debug_dbg_level_2_mt6993_debug_gen(
			CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF, g_debug_info_ptr);
		consys_print_log("[TOP_CR_C]", g_debug_info_ptr);
	#if 0 /* since pbus is not enable for this project, skip dumping related log. */
		consys_print_pbus_debug_dbg_level_2_mt6993_debug_gen(g_debug_info_ptr);
...
int consys_debug_top_power_status_mt6993(void)
{
	...
	consys_print_top_power_debug_dbg_level_0_mt6993_debug_gen(
			g_debug_top_power_info_ptr);
	consys_print_log("[CONN_SUBSYS_POWER]", g_debug_top_power_info_ptr);
	return ret;
}

void consys_debug_init_mt6993(void)
{
	g_debug_info_ptr =
		(struct conn_debug_info_mt6993 *)osal_malloc(sizeof(struct conn_debug_info_mt6993));
	if (g_debug_info_ptr == NULL) {
	...
	consys_debug_init_mt6993_debug_gen();
}

void consys_debug_deinit_mt6993(void)
{
	if (g_debug_info_ptr != NULL) {
	...
	consys_debug_deinit_mt6993_debug_gen();
}
