...
struct consys_base_addr conn_reg_mt6878;

static struct conn_debug_info_mt6878 static_debug_info;
static struct conn_debug_info_mt6878 *debug_info;
...
static void consys_print_power_debug(enum conninfra_bus_error_type level)
{
	...
	if (level >= CONNINFRA_BUS_LOG_LEVEL_HOST_ONLY) {
		consys_print_power_debug_dbg_level_0_mt6878_debug_gen(level, debug_info);
		consys_print_log("[CONN_POWER_A]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON) {
		consys_print_power_debug_dbg_level_1_mt6878_debug_gen(level, debug_info);
		consys_print_log("[CONN_POWER_B]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF) {
		consys_print_power_debug_dbg_level_2_mt6878_debug_gen(level, debug_info);
...
#define ATOP_DBG_DUMP_NUM 1
	...
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_ON) {
		consys_print_bus_debug_dbg_level_1_mt6878_debug_gen(level, debug_info);
		consys_print_log("[CONN_BUS_B]", debug_info);
	}
	if (level >= CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF) {
		consys_print_bus_debug_dbg_level_2_mt6878_debug_gen(level, debug_info);
...
int consys_print_debug_mt6878(enum conninfra_bus_error_type level)
{
	if (level > CONNINFRA_BUS_LOG_LEVEL_CONNINFRA_OFF) {
...
unsigned int consys_bus_hang_clock_detect_mt6878(void)
{
	unsigned int r = 0;
...
int consys_check_conninfra_on_domain_status_mt6878(void)
{
	unsigned int r1, r2;
	int ret = -1;
...
int consys_check_conninfra_bus_clock_status_mt6878(void)
{
	unsigned int r;
	unsigned int count = 0;

	while (count < 4) {
		r = consys_bus_hang_clock_detect_mt6878();
		if (r == 0x6)
			break;
...
int consys_check_conninfra_off_domain_status_mt6878(void)
{
	unsigned int r;
...
int consys_check_conninfra_irq_status_mt6878(void)
{
	int ret = -1;
...
void consys_debug_init_mt6878(void)
{
	...
	consys_debug_init_mt6878_debug_gen();
}

void consys_debug_deinit_mt6878(void)
{
	if (debug_info != NULL)
		debug_info = NULL;
	...
	consys_debug_deinit_mt6878_debug_gen();
}
