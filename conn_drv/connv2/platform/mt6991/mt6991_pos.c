...
static int consys_sema_acquire_timeout_mt6991(unsigned int index, unsigned int usec);
static int consys_sema_acquire_mt6991(unsigned int index);
static void consys_sema_release_mt6991(unsigned int index);
#ifndef CONFIG_FPGA_EARLY_PORTING
static const char *get_spi_sys_name(enum sys_spi_subsystem subsystem);
#endif

unsigned int consys_emi_set_remapping_reg_mt6991(
	phys_addr_t con_emi_base_addr,
	phys_addr_t md_shared_emi_base_addr,
	phys_addr_t gps_emi_base_addr)
{
	return consys_emi_set_remapping_reg_mt6991_gen(con_emi_base_addr, md_shared_emi_base_addr,
							gps_emi_base_addr, 16);
}

bool consys_is_rc_mode_enable_mt6991(void)
{
...
int consys_conninfra_on_power_ctrl_mt6991(unsigned int enable)
{
	int ret = 0;

#ifndef CONFIG_FPGA_EARLY_PORTING
#if MTK_CONNINFRA_CLOCK_BUFFER_API_AVAILABLE
	ret = consys_platform_spm_conn_ctrl_mt6991(enable);
#else
	ret = consys_conninfra_on_power_ctrl_mt6991_gen(1, 0, enable);
#endif
#endif
	if (enable)
		mdelay(5);
...
int consys_polling_chipid_mt6991(void)
{
	return consys_polling_chipid_mt6991_gen(NULL);
}
...
int connsys_d_die_cfg_mt6991(void)
{
	 ...
	 * Write to conninfra sysram: CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE(0x4005_3820)
	 */
	connsys_get_d_die_efuse_mt6991_gen(&d_die_efuse);
	CONSYS_REG_WRITE(
		CONN_INFRA_SYSRAM_SW_CR_D_DIE_EFUSE, d_die_efuse);

	connsys_d_die_cfg_mt6991_gen();
...
int connsys_spi_master_cfg_mt6991(unsigned int curr_status, unsigned int next_status)
{
	...
	connsys_wt_slp_top_ctrl_adie6686_mt6991_gen();
	return 0;
}
...
{
	return connsys_subsys_pll_initial_xtal_26000k_mt6991_gen();
}

int connsys_low_power_setting_mt6991(unsigned int curr_status, unsigned int next_status)
{
	if (curr_status == 0)
		connsys_low_power_setting_mt6991_gen();
...
{
	return consys_conninfra_wakeup_mt6991_gen();
}
...
int consys_sema_acquire_timeout_mt6991(unsigned int index, unsigned int usec)
{
	unsigned int i;
	...
	if (index >= CONN_SEMA_NUM_MAX)
		return CONN_SEMA_GET_FAIL;
	for (i = 0; i < usec; i++) {
		if (consys_sema_acquire_mt6991(index) == CONN_SEMA_GET_SUCCESS) {
...
void consys_sema_release_mt6991(unsigned int index)
{
...
static int consys_spi_read_nolock_mt6991(
...
int consys_spi_read_mt6991(
	enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	int ret = 0;
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout_mt6991(
		CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_err("[SPI READ] Require semaphore fail\n");
		return CONNINFRA_SPI_OP_FAIL;
	}

	ret = consys_spi_read_nolock_mt6991(subsystem, addr, data);

	consys_sema_release_mt6991(CONN_SEMA_RFSPI_INDEX);
	return ret;
}

static int consys_spi_write_nolock_mt6991(
	enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
#ifndef CONFIG_FPGA_EARLY_PORTING
	int check = 0;
...
int consys_spi_write_mt6991(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout_mt6991(
	...
	ret = consys_spi_write_nolock_mt6991(subsystem, addr, data);

	consys_sema_release_mt6991(CONN_SEMA_RFSPI_INDEX);
	return ret;
}

int consys_spi_update_bits_mt6991(
	enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data, unsigned int mask)
{
	...
	ret = consys_spi_read_nolock_mt6991(subsystem, addr, &curr_val);
	...
	if (change)
		ret = consys_spi_write_nolock_mt6991(subsystem, addr, new_val);
