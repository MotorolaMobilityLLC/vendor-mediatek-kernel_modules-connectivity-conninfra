...
#include "include/mt6886.h" /* For function declaration */
#include "include/mt6886_consys_reg.h" /* cr base address */
#include "include/mt6886_consys_reg_offset.h" /* cr offset */
...
unsigned int consys_emi_set_remapping_reg_mt6886(
	phys_addr_t con_emi_base_addr,
	phys_addr_t md_shared_emi_base_addr,
	phys_addr_t gps_emi_base_addr)
{
	return consys_emi_set_remapping_reg_mt6886_gen(con_emi_base_addr, md_shared_emi_base_addr,
							gps_emi_base_addr, 16);
}

int consys_conninfra_wakeup_mt6886(void)
{
	return consys_conninfra_wakeup_mt6886_gen();
}

int consys_conninfra_sleep_mt6886(void)
{
	return consys_conninfra_sleep_mt6886_gen();
}

int consys_polling_chipid_mt6886(void)
{
	return consys_polling_chipid_mt6886_gen(NULL);
}
					     ...
					     unsigned int next_status)
{
	...
	ret = connsys_adie_clock_buffer_setting_mt6886_gen(
...
{
	connsys_wt_slp_top_ctrl_adie6637_mt6886_gen();
	return 0;
}
...
int connsys_a_die_cfg_mt6886(void)
{
	...
	memset(&input, 0, sizeof(struct consys_plat_thermal_data_mt6886));

	connsys_a_die_switch_to_gpio_mode_mt6886_gen();
	connsys_adie_top_ck_en_ctl_mt6886_gen(1);
	connsys_a_die_cfg_deassert_adie_reset_mt6886_gen();

	if (consys_sema_acquire_timeout_mt6886(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT)
		== CONN_SEMA_GET_FAIL) {
	...
	check = connsys_a_die_cfg_read_adie_id_mt6886_gen(&adie_id, &hw_ver_id);
	if (check) {
		consys_sema_release_mt6886(CONN_SEMA_RFSPI_INDEX);
		return -1;
	}
	...
	connsys_a_die_efuse_read_get_efuse_info_mt6886_gen(sysram_efuse_list,
			&(input.slop_molecule), &(input.thermal_b), &(input.offset));
	pr_info("slop_molecule=[%d], thermal_b =[%d], offset=[%d]",
		input.slop_molecule, input.thermal_b, input.offset);
	update_thermal_data_mt6886(&input);

	connsys_a_die_cfg_PART2_mt6886_gen(hw_ver_id);

	consys_sema_release_mt6886(CONN_SEMA_RFSPI_INDEX);

	connsys_a_die_switch_to_conn_mode_mt6886_gen();

	conn_hw_env.is_rc_mode = consys_is_rc_mode_enable_mt6886();

	sleep_mode = consys_get_sleep_mode_mt6886();
	pr_info("sleep_mode = %d\n", sleep_mode);
	connsys_wt_slp_top_power_saving_ctrl_adie6637_mt6886_gen(adie_id, sleep_mode);
...
int connsys_afe_wbg_cal_mt6886(void)
{
	...
	if (first_cal == 0)
		return 0;
	first_cal = 0;
	return connsys_afe_wbg_cal_mt6886_gen(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT);
}

int connsys_subsys_pll_initial_mt6886(void)
{
	return connsys_subsys_pll_initial_xtal_26000k_mt6886_gen();
}
...
int connsys_low_power_setting_mt6886(unsigned int curr_status, unsigned int next_status)
{
	...
	if (curr_status == 0) {
		connsys_low_power_setting_mt6886_gen();
	} else {
		/* Not first on */
	}
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
				  ...
				  unsigned int *data)
{
	...
	CONSYS_REG_WRITE(CONN_REG_CONN_RF_SPI_MST_REG_ADDR + op->addr_cr,
...
int consys_spi_read_mt6886(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	int ret = 0;
	...
	ret = consys_spi_read_nolock_mt6886(subsystem, addr, data);
...
int consys_spi_write_mt6886(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	int ret = 0;
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout_mt6886(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT)
	...
	ret = consys_spi_write_nolock_mt6886(subsystem, addr, data);
				  ...
				  unsigned int data, unsigned int mask)
{
	...
	ret = consys_spi_read_nolock_mt6886(subsystem, addr, &curr_val);
	...
	if (change) {
		ret = consys_spi_write_nolock_mt6886(subsystem, addr, new_val);
	}
...
int consys_subsys_status_update_mt6886(bool on, int radio)
{
	...
	if (consys_sema_acquire_timeout_mt6886(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX,
	...
	consys_sema_release_mt6886(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX);
	...
	unsigned int reg_offset, unsigned int value_offset, unsigned int size)
{
	...
	data2 = data & (~reg_mask);
	data2 = (data2 | value);
	consys_spi_write_nolock_mt6886(subsystem, addr, data2);
}
...
{
	return connsys_adie_top_ck_en_ctl_mt6886_gen(onoff);
}
