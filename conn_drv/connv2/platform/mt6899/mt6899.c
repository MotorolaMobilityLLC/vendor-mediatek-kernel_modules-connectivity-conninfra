...
static struct consys_plat_thermal_data_mt6899 g_consys_plat_therm_data;
...
unsigned long mt6899_power_state_dump_data[POWER_STATE_DUMP_DATA_SIZE];

int consys_co_clock_type_mt6899(void)
{
	return conn_hw_env.clock_type;
}

int consys_enable_power_dump_mt6899(void)
{
...
#define CONN_TICK_TO_SEC(TICK) (TICK / CONN_32K_TICKS_PER_SEC)
	...
	if (print_log) {
		mt6899_power_state_dump_data[0] = round;
		mt6899_power_state_dump_data[1] = CONN_TICK_TO_SEC(conninfra_sleep_time);
		mt6899_power_state_dump_data[2] = CONN_TICK_TO_SEC((conninfra_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[3] = conninfra_sleep_cnt;
		mt6899_power_state_dump_data[4] = CONN_TICK_TO_SEC(wf_sleep_time);
		mt6899_power_state_dump_data[5] = CONN_TICK_TO_SEC((wf_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[6] = wf_sleep_cnt;
		mt6899_power_state_dump_data[7] = CONN_TICK_TO_SEC(bt_sleep_time);
		mt6899_power_state_dump_data[8] = CONN_TICK_TO_SEC((bt_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[9] = bt_sleep_cnt;
		mt6899_power_state_dump_data[10] = CONN_TICK_TO_SEC(gps_sleep_time);
		mt6899_power_state_dump_data[11] = CONN_TICK_TO_SEC((gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[12] = gps_sleep_cnt;
		mt6899_power_state_dump_data[13] = CONN_TICK_TO_SEC(t_conninfra_sleep_time);
		mt6899_power_state_dump_data[14] = CONN_TICK_TO_SEC((t_conninfra_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[15] = t_conninfra_sleep_cnt;
		mt6899_power_state_dump_data[16] = CONN_TICK_TO_SEC(t_wf_sleep_time);
		mt6899_power_state_dump_data[17] = CONN_TICK_TO_SEC((t_wf_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[18] = t_wf_sleep_cnt;
		mt6899_power_state_dump_data[19] = CONN_TICK_TO_SEC(t_bt_sleep_time);
		mt6899_power_state_dump_data[20] = CONN_TICK_TO_SEC((t_bt_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[21] = t_bt_sleep_cnt;
		mt6899_power_state_dump_data[22] = CONN_TICK_TO_SEC(t_gps_sleep_time);
		mt6899_power_state_dump_data[23] = CONN_TICK_TO_SEC((t_gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6899_power_state_dump_data[24] = t_gps_sleep_cnt;
		...
		ret = snprintf(buf_p, buf_sz,
			...
			mt6899_power_state_dump_data[0],
			mt6899_power_state_dump_data[1],
			mt6899_power_state_dump_data[2],
			mt6899_power_state_dump_data[3],
			mt6899_power_state_dump_data[4],
			mt6899_power_state_dump_data[5],
			mt6899_power_state_dump_data[6],
			mt6899_power_state_dump_data[7],
			mt6899_power_state_dump_data[8],
			mt6899_power_state_dump_data[9],
			mt6899_power_state_dump_data[10],
			mt6899_power_state_dump_data[11],
			mt6899_power_state_dump_data[12],
			mt6899_power_state_dump_data[13],
			mt6899_power_state_dump_data[14],
			mt6899_power_state_dump_data[15],
			mt6899_power_state_dump_data[16],
			mt6899_power_state_dump_data[17],
			mt6899_power_state_dump_data[18],
			mt6899_power_state_dump_data[19],
			mt6899_power_state_dump_data[20],
			mt6899_power_state_dump_data[21],
			mt6899_power_state_dump_data[22],
			mt6899_power_state_dump_data[23],
			mt6899_power_state_dump_data[24]);
		if (ret > 0)
			pr_info("%s", buf_p);
...
int consys_reset_power_state_mt6899(void)
{
	return consys_power_state_dump(NULL, 0, 0);
}

int consys_power_state_dump_mt6899(char *buf, unsigned int size)
{
	return consys_power_state_dump(buf, size, 1);
}

unsigned int consys_get_hw_ver_mt6899(void)
{
	return CONN_HW_VER;
}

void update_thermal_data_mt6899(struct consys_plat_thermal_data_mt6899 *input)
{
	memcpy(&g_consys_plat_therm_data, input, sizeof(struct consys_plat_thermal_data_mt6899));
}

#define PRINT_THERMAL_LOG 1
static int calculate_thermal_temperature(int y)
{
	struct consys_plat_thermal_data_mt6899 *data = &g_consys_plat_therm_data;
	int t;
	int const_offset = 30;
...
int consys_thermal_query_mt6899(void)
{
...
#define CONN_GPT2_CTRL_AP_EN	0x38
	...
	connsys_adie_top_ck_en_ctl_mt6899(true);

	/* Hold Semaphore, TODO: may not need this, because
	 * thermal cr separate for different
	 */
	if (consys_sema_acquire_timeout_mt6899(CONN_SEMA_THERMAL_INDEX, CONN_SEMA_TIMEOUT)
		== CONN_SEMA_GET_FAIL) {
		pr_err("[THERM QRY] Require semaphore fail\n");
		connsys_adie_top_ck_en_ctl_mt6899(false);
		iounmap(addr);
		return -1;
	}
	...
	consys_sema_release_mt6899(CONN_SEMA_THERMAL_INDEX);
	connsys_adie_top_ck_en_ctl_mt6899(false);
...
unsigned int consys_adie_detection_mt6899(unsigned int drv_type)
{
	return ADIE_6637;
}

unsigned int consys_get_adie_chipid_mt6899(void)
{
	return ADIE_6637;
}

void consys_set_mcu_control_mt6899(int type, bool onoff)
{
...
int consys_pre_cal_backup_mt6899(unsigned int offset, unsigned int size)
{
...
int consys_pre_cal_restore_mt6899(void)
{
	int i;
	...
	connsys_adie_top_ck_en_ctl_mt6899(true);
	/* Acquire semaphore */
	if (consys_sema_acquire_timeout_mt6899(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT)
		== CONN_SEMA_GET_FAIL) {
	 ...
	 */
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xb04, 0xfe000000);
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xb08, 0xe0000000);
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xa04, 0xffffffff);
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xaf4, 0xffffffff);
	/* Write CR back, SYS_SPI_WF & SYS_SPI_WF1 */
	for (i = 0; i < mt6637_backup_cr_number; i++) {
		consys_spi_write_nolock_mt6899(
			SYS_SPI_WF,
	 ...
	 */
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xb04, 0x88000000);
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xb08, 0x00000000);
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xa04, 0x00000000);
	consys_spi_write_nolock_mt6899(SYS_SPI_TOP, 0xaf4, 0x00000000);
	/* Release semaphore */
	consys_sema_release_mt6899(CONN_SEMA_RFSPI_INDEX);
	/* Disable a-die top ck en */
	connsys_adie_top_ck_en_ctl_mt6899(false);
	return 0;
}

int consys_pre_cal_clean_data_mt6899(void)
{
	pr_info("[%s]", __func__);
	mt6637_backup_cr_number = 0;
