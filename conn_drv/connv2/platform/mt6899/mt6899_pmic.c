...
********************************************************************************
*/
static int consys_plt_pmic_get_from_dts_mt6899(struct platform_device*, struct conninfra_dev_cb*);

static int consys_plt_pmic_common_power_ctrl_mt6899(unsigned int,
					unsigned int curr_status, unsigned int next_status);
static int consys_plt_pmic_common_power_low_power_mode_mt6899(unsigned int,
					unsigned int curr_status, unsigned int next_status);
static int consys_plt_pmic_wifi_power_ctrl_mt6899(unsigned int);
static int consys_plt_pmic_bt_power_ctrl_mt6899(unsigned int);
static int consys_plt_pmic_gps_power_ctrl_mt6899(unsigned int);
static int consys_plt_pmic_fm_power_ctrl_mt6899(unsigned int);
static int consys_pmic_vcn33_1_power_ctl_mt6899_lg(bool);
static int consys_pmic_vcn33_1_power_ctl_mt6899_rc(bool);
static int consys_pmic_vcn33_2_power_ctl_mt6899_lg(bool);
static int consys_pmic_vcn33_2_power_ctl_mt6899_rc(bool);
static int consys_pmic_vant18_power_ctl_mt6899(bool);
...
const struct consys_platform_pmic_ops g_consys_platform_pmic_ops_mt6899 = {
	...
	.consys_pmic_common_power_ctrl = consys_plt_pmic_common_power_ctrl_mt6899,
	.consys_pmic_common_power_low_power_mode = consys_plt_pmic_common_power_low_power_mode_mt6899,
	.consys_pmic_wifi_power_ctrl = consys_plt_pmic_wifi_power_ctrl_mt6899,
	.consys_pmic_bt_power_ctrl = consys_plt_pmic_bt_power_ctrl_mt6899,
...
int consys_plt_pmic_common_power_ctrl_mt6899(unsigned int enable,
					unsigned int curr_status, unsigned int next_status)
{
	int sleep_mode;
	int ret = 0;
	...
	if (enable) {
		...
		/* 1. set PMIC VS2 BUCK VOSEL 1.45V
		 * 2. delay 15us
		 * 3. set PMIC VS2  BUCK  Voter_EN = 1
		 * (by "standard kernal PMIC API" and "PMIC table") from Ponsot workaround
		 */
		regmap_write(r, MT6363_TOP_VRCTL_VOSEL_VBOCK0, 0x74);
		udelay(15);
		regmap_write(r, MT6363_BUCK_VS2_VOTER_CON0_SET_ADDR, 0x10);
		...
		*/
		...
		/* vant18 is enabled in consys_plt_pmic_common_power_low_power_mode_mt6899 */
		/* Please refer to POS for more information */
		consys_pmic_vant18_power_ctl_mt6899(0);

		/* vcn33_1/2 is enabled in consys_plt_pmic_common_power_low_power_mode_mt6899 */
		if (consys_is_rc_mode_enable_mt6899()) {
			consys_pmic_vcn33_2_power_ctl_mt6899_rc(0);
			consys_pmic_vcn33_1_power_ctl_mt6899_rc(0);
		}
		...
		/* 1. set PMIC VS2 BUCK VOSEL 1.35V
		 * 2. set PMIC VS2  BUCK  Voter_EN = 0
		 * (by "standard kernal PMIC API" and "PMIC table") from Ponsot workaround
		 */
		regmap_write(r, MT6363_TOP_VRCTL_VOSEL_VBOCK0, 0x6C);
		regmap_write(r, MT6363_BUCK_VS2_VOTER_CON0_CLR_ADDR, 0x10);
...
int consys_plt_pmic_common_power_low_power_mode_mt6899(unsigned int enable,
					unsigned int curr_status, unsigned int next_status)
{
	int ret = 0;
	int sleep_mode;
	...
	if (consys_is_rc_mode_enable_mt6899()) {
		...
		if (sleep_mode == 1 || sleep_mode == 3) {
			...
			/* 2. set PMIC VRFIO18 LDO SW_OP_EN =0, SW_EN = 0, SW_LP =0 */
			ret = regulator_disable(reg_VRFIO18);
			if (ret)
				pr_notice("%s regulator_disable err: %d", __func__, ret);
			regulator_set_mode(reg_VRFIO18, REGULATOR_MODE_NORMAL);
	...
	if (consys_is_rc_mode_enable_mt6899()) {
		consys_pmic_vcn33_1_power_ctl_mt6899_rc(enable);
		consys_pmic_vcn33_2_power_ctl_mt6899_rc(enable);
	}
	consys_pmic_vant18_power_ctl_mt6899(enable);
	return ret;
}

int consys_plt_pmic_wifi_power_ctrl_mt6899(unsigned int enable)
{
	int ret;
	...
	ret = consys_pmic_vcn33_1_power_ctl_mt6899_lg(enable);
	if (ret)
		pr_info("%s VCN33_1 fail\n", (enable? "Enable" : "Disable"));

	ret = consys_pmic_vcn33_2_power_ctl_mt6899_lg(enable);
	if (ret)
...
int consys_plt_pmic_bt_power_ctrl_mt6899(unsigned int enable)
{
	/* necessary in legacy mode only */
	if (consys_is_rc_mode_enable_mt6899())
		return 0;
	return consys_pmic_vcn33_1_power_ctl_mt6899_lg(enable);
}

int consys_plt_pmic_gps_power_ctrl_mt6899(unsigned int enable)
{
	return 0;
}

int consys_plt_pmic_fm_power_ctrl_mt6899(unsigned int enable)
{
	return 0;
}

static int consys_pmic_vcn33_1_power_ctl_mt6899_rc(bool enable)
{
	int sleep_mode;
...
static int consys_pmic_vcn33_1_power_ctl_mt6899_lg(bool enable)
{
	struct regmap *r = g_regmap_mt6368;
...
static int consys_pmic_vcn33_2_power_ctl_mt6899_lg(bool enable)
{
	struct regmap *r = g_regmap_mt6368;
	int ret = 0;
...
static int consys_pmic_vcn33_2_power_ctl_mt6899_rc(bool enable)
{
	int sleep_mode;
	struct regmap *r = g_regmap_mt6368;
	int ret;
...
static int consys_pmic_vant18_power_ctl_mt6899(bool enable)
{
	struct regmap *r = g_regmap_mt6368;
	int ret = 0;
	...
	if (!enable) {
		...
		if (consys_is_rc_mode_enable_mt6899() == 0 && regulator_is_enabled(reg_VANT18)) {
			ret = regulator_disable(reg_VANT18);
			if (ret)
				pr_notice("%s regulator_disable err:%d", __func__, ret);
		} else if (consys_is_rc_mode_enable_mt6899() == 1 && with_1v8_efem_support) {
			/*
	...
	if (consys_is_rc_mode_enable_mt6899()) {
		if (with_1v8_efem_support) {
			/*
			 * 1. set PMIC VANT18 LDO PMIC HW mode control by PMRC_EN[6][7][8]
			 * (by ""standard kernal PMIC API"" and ""PMIC table"")
...
static int consys_plt_pmic_event_notifier_mt6899(unsigned int id, unsigned int event)
{
...
#define AWF_DUMP_NUM 3
	...
	connsys_adie_top_ck_en_ctl_mt6899(1);
	consys_hw_spi_update_bits(SYS_SPI_TOP, 0x580, 0x00, 0x10);
	connsys_adie_top_ck_en_ctl_mt6899(0);
	dump_adie_cr(SYS_SPI_TOP, adie_top_cr_list, ATOP_DUMP_NUM, "A-die TOP");
...
void consys_pmic_debug_log_mt6899(void)
{
	struct regmap *r = g_regmap_mt6363;
