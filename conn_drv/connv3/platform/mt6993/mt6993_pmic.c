...
#include "consys_reg_util.h"
#include "mt6376.h"
#include "connv3_mt6688_reg_def.h"
...
static struct pmic_addr_mask_value mt6688_BLE[] =
{
	{0x0004, 0xFF, 0x88},
	{0x0410, 0xFF, 0x10},
	{0x0606, 0xFF, 0x00},
...
static struct pmic_addr_mask_value mt6661AP_BLE[] =
{
	...
	/*
	 * BLE GPIO related pre-setting for MT6661AP
	 * UDS_EN : GPIO1
	 * FaultB_CONN : GPIO3
	 * EN_BLE_OUT: AGPIO2
	 */
	{0xCF, 0xC0, 0x40},	// GPIO1 mode reset when (btrstb == 0 & wdtrstb == 0)
	{0xD0, 0xC, 0x4},	// GPIO3 mode reset when (btrstb == 0 & wdtrstb == 0)
	{0xD1, 0xC, 0x8},	// AGPIO2 mode reset when (PORSTB == 0)
	{0xD2, 0xC0, 0x40},	// GPIO1 status reset when (btrstb == 0 & wdtrstb == 0)
	{0xD3, 0xC, 0x4},	// GPIO3 status reset when (btrstb == 0 & wdtrstb == 0)
	{0xD4, 0xC, 0x8},	// AGPIO2 status reset when (PORSTB == 0)
...
static struct pmic_addr_mask_value mt6661BP_BLE[] =
{
	{0xA2F, 0xFF, 0x29}, // unlock cpskey
	{0xA30, 0xFF, 0x47}, // unlock cpskey
	{0x3B4, 0xFF, 0x9E}, // tma key
	...
	/*
	 * BLE GPIO related pre-setting for MT6661BP
	 * UDS_EN : GPIO1
	 * EN_BLE_OUT: AGPIO2
	 */
	{0xCF, 0xC0, 0x40},	// GPIO1 mode reset when (btrstb == 0 & wdtrstb == 0)
	{0xD1, 0xC, 0x4},	// AGPIO2 mode reset when (btrstb == 0 & wdtrstb == 0)
	{0xD2, 0xC0, 0x40},	// GPIO1 status reset when (btrstb == 0 & wdtrstb == 0)
	{0xD4, 0xC, 0x4},	// AGPIO2 status reset when (btrstb == 0 & wdtrstb == 0)
...
static struct pmic_addr_mask_value mt6661AP_UDS[] =
{
	{0xA2F, 0xFF, 0x29}, // unlock cpskey
	{0xA30, 0xFF, 0x47}, // unlock cpskey
	{0x3B4, 0xFF, 0x9E}, // tma key
...
static struct pmic_addr_mask_value mt6661BP_UDS[] =
{
	{0xA2F, 0xFF, 0x29}, // unlock cpskey
	{0xA30, 0xFF, 0x47}, // unlock cpskey
	{0x3B4, 0xFF, 0x9E}, // tma key
...
static struct pmic_addr_mask_value mt6688_UDS_EN[] =
{
	{0x0610,	0xFF,	0x10},
	{0x060B,	0xFF,	0x10},
	{0x0613,	0xFF,	0x00},
...
static struct pmic_addr_mask_value mt6688_initial_setting[] =
{
	{0x0610,	0xFF,	0x10},
	{0x060B,	0xFF,	0x10},
	{0x0613,	0xFF,	0x00},
...
static void pmic_backup_array_read_back(struct regmap *regmap, struct pmic_addr_mask_value *setting_array, u32 array_size)
{
	int i = 0;
...
#define RG_ONE_LINE 5
#define TMP_RG_SIZE 20
#define TMP_STRING_SIZE (TMP_RG_SIZE*RG_ONE_LINE + 10)
static char tmp_buf[TMP_RG_SIZE] = {'\0'};
static char tmp_buf_one_line[TMP_STRING_SIZE] = {'\0'};

static void pmic_backup_and_write_array(struct regmap *regmap, struct pmic_addr_mask_value *setting_array, u32 array_size, char* tag)
{
	u32 i, j = 0;
	char *log_tag = tag;

	if (tag == NULL)
		log_tag = "[pmic_backup_and_write_array]";

	for (i = 0; i < array_size ; i++) {
		regmap_read(regmap, (setting_array+i)->addr, &((setting_array+i)->bk_value));
		if (snprintf(tmp_buf, TMP_RG_SIZE, " 0x%04x=0x%04x,", (setting_array+i)->addr, (setting_array+i)->bk_value) >= 0) {
			strncat(tmp_buf_one_line, tmp_buf, strlen(tmp_buf));
		}

		if ((i + 1) % RG_ONE_LINE == 0 || i == array_size - 1) {
			pr_info("[%s][%d-%d] %s\n", tag, j, i, tmp_buf_one_line);
			memset(tmp_buf_one_line, '\0', sizeof(tmp_buf_one_line));
			j = i + 1;
		}
	}
...
int connv3_plt_pmic_initial_setting_mt6993(struct platform_device *pdev, struct connv3_dev_cb* dev_cb);
int connv3_plt_pmic_common_power_ctrl_mt6993(u32 enable);
int connv3_plt_pmic_vsel_ctrl_mt6993(u32 enable);
int connv3_plt_pmic_parse_state_mt6993(char *buffer, int buf_sz);
int connv3_plt_pmic_get_connsys_chip_info_mt6993(char *connsys_ecid, int connsys_ecid_size);
int connv3_plt_pmic_get_connsys_adie_chip_info_mt6993(char *connsys_adie_chip_info, int connsys_adie_chip_info_size);
...
const struct connv3_platform_pmic_ops g_connv3_platform_pmic_ops_mt6993 = {
	...
	.pmic_common_power_ctrl = connv3_plt_pmic_common_power_ctrl_mt6993,
	.pmic_vsel_ctrl = connv3_plt_pmic_vsel_ctrl_mt6993,
	.pmic_parse_state = connv3_plt_pmic_parse_state_mt6993,
	.pmic_get_connsys_chip_info = connv3_plt_pmic_get_connsys_chip_info_mt6993,
	.pmic_get_connsys_adie_chip_info = connv3_plt_pmic_get_connsys_adie_chip_info_mt6993,
...
int g_faultb_gpio_mt6993 = -1, g_pmic_en_gpio_mt6993 = -1;
...
#define PMIC_EXCEPTION_STRING_LEN	40
	...
	if (g_spurious_pmic_exception_mt6993) {
		pr_info("[%s] g_spurious_pmic_exception_mt6993\n", __func__);
		return IRQ_HANDLED;
	}

	pr_notice("[%s] Get PMIC FaultB interrupt\n", __func__);
	...
	schedule_work(&g_pmic_faultb_work_mt6993);
...
static void check_faultb_status(struct work_struct *work)
{
	...
	if ((g_faultb_gpio_mt6993 != -1) && (g_pmic_en_gpio_mt6993 != -1)) {
		faultb_level = gpio_get_value(g_faultb_gpio_mt6993);
		pr_info("[%s] PMIC_EN=%d, faultb=%d\n",
			__func__, gpio_get_value(g_pmic_en_gpio_mt6993), faultb_level);
		if (faultb_level == 1)
			return;
	}
...
int connv3_plt_pmic_common_power_ctrl_mt6993(u32 enable)
{
	...
	if (enable == 1 && pmic_enabled == 0) {
		...
		if (!IS_ERR(pinctrl_set)) {
			...
			pr_notice("[%s] fail to get \"connsys-pin-pmic-en-set\"",  __func__);
		}
		mdelay(20);
		...
		if (!IS_ERR(faultb_set)) {
			...
			if (ret) {
				pr_notice("[%s] faultb on fail, %d", __func__, ret);
				ret = 0;
			}
		} else {
			pr_notice("[%s] fail to get \"connsys-pin-pmic-faultb-enable\"",  __func__);
		}

		g_spurious_pmic_exception_mt6993 = 0;
		pmic_enabled = 1;
		...
		g_spurious_pmic_exception_mt6993 = 1;
		...
		if (!IS_ERR(faultb_set)) {
			ret = pinctrl_select_state(g_pinctrl_ptr, faultb_set);
			if (ret)
				pr_notice("[%s] faultb off fail, %d", __func__, ret);
		} else {
			pr_notice("[%s] fail to get \"connsys-pin-pmic-faultb-default\"",  __func__);
		}
		...
		if (!IS_ERR(pinctrl_set)) {
			...
			pr_notice("[%s] fail to get \"connsys-pin-pmic-en-clr\"",	__func__);
		}
...
#define PMIC_DUMP_REGISTER_SIZE	240
	...
	if (!buffer){
		pr_notice("[%s] PMIC dump register is NULL\n", __func__);
		return -1;
	}
...
int connv3_plt_pmic_fmd_setting_mt6993(u32 enable)
{
	...
	pmic_backup_and_write_array(g_connv3_regmap_mt6661_4, mt6661AP_BLE, sizeof(mt6661AP_BLE) / sizeof(mt6661AP_BLE[0]), NULL);

	/* mt6661AP UDS part */
	pmic_backup_and_write_array(g_connv3_regmap_mt6661_4, mt6661AP_UDS, sizeof(mt6661AP_UDS) / sizeof(mt6661AP_UDS[0]), NULL);

	/* mt6661BP setting on 20241009 */
	pmic_backup_and_write_array(g_connv3_regmap_mt6661_3, mt6661BP_BLE, sizeof(mt6661BP_BLE) / sizeof(mt6661BP_BLE[0]), NULL);

	/* mt6661BP UDS part */
	pmic_backup_and_write_array(g_connv3_regmap_mt6661_3, mt6661BP_UDS, sizeof(mt6661BP_UDS) / sizeof(mt6661BP_UDS[0]), NULL);
...
int connv3_plt_pmic_initial_setting_mt6993(
	struct platform_device *pdev, struct connv3_dev_cb* dev_cb)
{
	...
	INIT_WORK(&g_pmic_faultb_work_mt6993, check_faultb_status);
	g_faultb_gpio_mt6993 = of_get_named_gpio(pdev->dev.of_node, "mt6376-gpio", 0);
	g_pmic_en_gpio_mt6993 = of_get_named_gpio(pdev->dev.of_node, "mt6376-gpio", 1);
	...
	if (ret)
		pr_notice("[%s][%d], fail to enable FaultB wake, ret=%d\n", __func__, __LINE__, ret);
