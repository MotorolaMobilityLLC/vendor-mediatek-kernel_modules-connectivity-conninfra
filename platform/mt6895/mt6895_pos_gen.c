...
void consys_set_if_pinmux_mt6895_gen(unsigned int enable)
{
	...
	if (enable == 1) {
		/* set pinmux for the interface between D-die and A-die (Aux1) */
			...
			CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
				CONSYS_GEN_GPIO_MODE21_OFFSET_ADDR, 0x11101111, 0x77707777);
		#endif
		...
		/* set pinmux for the interface between D-die and A-die (Aux0) */
			...
			CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
				CONSYS_GEN_GPIO_MODE21_OFFSET_ADDR, 0x0, 0x77707777);
		#endif
...
int connsys_a_die_switch_to_gpio_mode_mt6895_gen(void)
{
	...
	if (consys_co_clock_type_mt6895() != CONNSYS_CLOCK_SCHEMATIC_52M_COTMS) {
...
void connsys_afe_sw_patch_mt6895_gen(void)
{
	...
	/* note that this CR must be backuped and restored by command batch engine */
		...
		CONSYS_REG_WRITE(CONN_AFE_CTL_BASE +
			CONSYS_GEN_RG_WBG_PLL_05_OFFSET_ADDR, 0xE3080409);
	#endif
}
